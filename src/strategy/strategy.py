...
class Strategy:
    commission = 0.0007  # 7 basis points as default

    def __init__(self, source_df, window_size, threshold, target='Price', price='Price', long_short="long", condition="higher", commission=None):
    ...
    def _add_position(self, df, signal, signal_type, threshold, long_short, condition):
        ...
        df['Profit'] = df['Position'].shift(1) * df['Changes'] - np.abs(df['Position'].diff()).fillna(
            0) * self.commission
